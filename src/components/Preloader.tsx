...
export function Preloader({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[9999] bg-white flex flex-col items-center justify-center transition-opacity duration-600 ${phase === 'fading' ? 'opacity-0 pointer-events-none' : 'opacity-100'
        }`}
    >
      ...
      <div className="mt-10 w-64 h-[1px] bg-slate-100 overflow-hidden">
        <div className="preloader-line h-full bg-gradient-to-r from-transparent via-blue-500 to-transparent" />
      </div>

      {/* Footer text */}
      <p
        className="preloader-text mt-6 text-[10px] text-slate-400 uppercase tracking-[0.4em] font-sans"
